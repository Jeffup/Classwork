...
public:
	...
	FibNode(T value,T index) :key(value), index(index), degree(0), marked(false),
...
public:
	...
	// 将斐波那契堆中键值oldkey更新为newkey    *关键
	void update(T index, T newkey);
...
private:
	...
	FibNode<T>* search(FibNode<T>* root, T index);
	FibNode<T>* search(T index);
	// 删除结点node
...
template <class T>
void FibHeap<T>::update(T index,T newkey)
{
	FibNode<T>* node;
 ...
 */
template <class T>
FibNode<T>* FibHeap<T>::search(FibNode<T>* root, T index)
{
	...
	do
	{
		...
		{
			if ((p = search(t->child, index)) != NULL)
				break;
		}
		t = t->right;
	} while (t != root);
...
{
	return search(index) != NULL ? true : false;
}
